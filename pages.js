...
function renderSettings() {
  ...
  const fszOpts = [
    { id:'sm', label:'小',   px:'13px' },
    { id:'md', label:'標準', px:'16px' },
...
// ────────────────────────────────────────────
function renderExam() {
  let h = `<div style="font-size:.7rem;color:var(--txt3);line-height:1.9;margin-bottom:14px;padding:10px 12px;background:var(--card);border-left:3px solid var(--gold2)">
    以下考級資料僅供參考，請以各機構官方網站公佈為準。
  </div>`;

  EXAMS.forEach(ex => {
    h += `<div class="card gold" style="margin-bottom:10px">
      <div style="font-family:var(--KAI);font-size:.9rem;color:var(--gold);margin-bottom:4px">${ex.name}</div>
      <div style="font-size:.67rem;color:var(--jade);letter-spacing:.06em;margin-bottom:6px">${ex.grades}</div>
      <div style="font-size:.72rem;color:var(--txt2);line-height:1.7;margin-bottom:7px">${ex.note}</div>
      <div style="display:flex;justify-content:space-between;align-items:center">
        <div style="font-size:.56rem;color:var(--txt3)">來源：${ex.src}</div>
        <a href="${ex.url}" target="_blank" style="font-size:.63rem;color:var(--gold2);border:1px solid var(--bdr2);padding:3px 9px;text-decoration:none;letter-spacing:.06em">官網 ↗</a>
      </div>
    </div>`;
  });
    ...
    sec.items.forEach(item => {
      h += `<div class="card" style="margin-bottom:8px">
        <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:10px">
          <div style="flex:1">
            <div style="font-size:.8rem;color:var(--txt);margin-bottom:3px">${item.title}</div>
            <div style="font-size:.66rem;color:var(--txt3);line-height:1.6">${item.note}</div>
          </div>
          <a href="${item.url}" target="_blank"
            style="font-size:.62rem;color:var(--gold2);border:1px solid var(--bdr2);padding:3px 9px;text-decoration:none;white-space:nowrap;flex-shrink:0">開啟 ↗</a>
        </div>
      </div>`;
    });
  });
